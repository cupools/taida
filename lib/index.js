...
exports.default = function (options) {
  if (!(0, _lint2.default)(options)) {
    return _promise2.default.reject('exit for unexpect options');
  }

  var pattern = options.pattern;
  var alternate = options.alternate;

  _tinifier2.default.apikey.alternate = alternate === undefined || !!alternate;
  ...
  var handleError = function handleError(err) {
    return _log2.default.error(err.message);
  };
  ...
  var resourcesP = resources.map(wrapImg).map(compressP).map(function (p) {
    return p.then(outputP(options.dest)).catch(handleError);
  });
