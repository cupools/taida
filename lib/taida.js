...
var taida = function taida(buffer) {
  ...
  return compress(_key, buffer).then(handleResult(buffer)).catch(handleError(_key, buffer, fallback));
};

function compress(_key, buffer) {
  return !_key ? _promise2.default.reject(new Error('failed for no usable apikey')) : new _promise2.default(function (resolve, reject) {
