...
exports.default = {
  ...
  set alternate(alternate) {
    return this.config({ apikeys: this.apikeys, alternate: alternate }).__alternate;
  },
  ...
  get: function get() {
    var now = Date.now();
    var revise = this.apikeys.map(function (item) {
      return !item.valid && now - item.date > 1 * 24 * 3600 * 1e3 ? (0, _extends3.default)({}, item, {
        valid: true,
        date: now
      }) : item;
    });
    ...
    this.config({ apikeys: revise, alternate: alternate });
